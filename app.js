...
function renderTopbar(route) {
  ...
  if (route.view === "atlas") {
    ...
    centerEl.innerHTML =
      ...
      '<span class="dot"></span>' +
      '<span class="breadcrumb-item">' + CHESS_DATA.clusters.length + ' regions · ' + Object.keys(CHESS_DATA.dossiers).length + ' dossiers</span>';
  } else if (route.view === "dossier") {
    const d = CHESS_DATA.dossiers[route.id];
    if (!d) return;
    const cluster = CHESS_DATA.clusters.find(function(c) { return c.id === d.cluster_id; });
    const clusterLabel = d.trans_geographic ? "Trans-geographic" : (cluster ? cluster.label : "");
    leftEl.innerHTML = '<a class="back-to-atlas" href="#atlas">← Atlas</a>';
    centerEl.innerHTML =
      '<a class="breadcrumb-item breadcrumb-link" href="#atlas">Atlas</a>' +
      '<span class="dot"></span>' +
      '<span class="breadcrumb-item">' + clusterLabel + '</span>' +
      '<span class="dot"></span>' +
...
function renderAtlasHTML() {
  ...
  const clusterMarkers = CHESS_DATA.clusters.map(function(c) {
    ...
    const dataAttr = firstDossierId ? ' data-first-dossier="' + firstDossierId + '"' : "";
    const countLabel = hasData ? '<text class="cluster-count" y="5" text-anchor="middle">' + count + '</text>' : "";

    return '<g class="cluster-marker ' + (hasData ? "has-data" : "empty") + '"' +
      ' data-cluster-id="' + c.id + '"' + dataAttr +
  ...
  const orbitalItems = transIds.map(function(id, i) {
  ...
  return '<section class="atlas-view">' +
    ...
    '<div class="atlas-map-wrap">' +
      '<svg class="atlas-map" viewBox="0 0 ' + mapW + ' ' + mapH + '" preserveAspectRatio="xMidYMid meet">' +
        '<ellipse cx="' + (mapW/2) + '" cy="' + (mapH/2) + '" rx="' + (mapW * 0.46) + '" ry="' + (mapH * 0.58) + '" fill="none" stroke="#d9d4c6" stroke-width="1" stroke-dasharray="3,4" opacity="0.5"/>' +
    ...
    '<div class="atlas-legend">' +
      ...
      '<div class="legend-group"><span class="legend-dot" style="background:#c4bfb1"></span><span>Region without dossiers</span></div>' +
      '<div class="legend-group"><span class="legend-dot" style="background:#5b21b6"></span><span>Trans-geographic (orbital)</span></div>' +
    '</div>' +
    '</section>';
}

function wireAtlasInteractions() {
  document.querySelectorAll(".cluster-marker.has-data").forEach(function(el) {
    el.addEventListener("click", function() {
      const id = el.getAttribute("data-first-dossier");
      if (id) navigate({ view: "dossier", id: id });
    });
  });
  ...
  return '<div class="dossier-view">' +
    '<aside class="chat-panel">' +
      ...
      '<div class="chat-messages">' +
        ...
        d.chat.map(renderMessage).join("") +
      '</div>' +
      '<div class="chat-input-wrap">' +
        '<div class="chat-input-box">' +
          '<textarea placeholder="Ask a follow-up, or start a new analysis…"></textarea>' +
          '<div class="chat-input-actions">' +
            '<button class="send-btn">Run <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M5 12h14M13 5l7 7-7 7"/></svg></button>' +
          '</div>' +
        '</div>' +
      '</div>' +
    '</aside>' +
...
function getSimpleContinentsPath(w, h) {
  function pts(coords) {
    return coords.map(function(c) {
      const p = projectToSVG(c[0], c[1], w, h);
  ...
  const nAmerica = pts([[-168,67],[-140,70],[-95,72],[-70,60],[-60,47],[-82,26],[-98,18],[-117,32],[-125,50],[-150,59],[-168,67]]);
  const sAmerica = pts([[-80,12],[-50,5],[-40,-20],[-60,-55],[-73,-52],[-80,-20],[-82,-5],[-80,12]]);
  const europe = pts([[-10,36],[5,36],[15,37],[28,38],[40,43],[60,55],[55,70],[28,71],[10,58],[-10,50],[-10,36]]);
  const africa = pts([[-18,15],[10,35],[32,32],[50,12],[51,-5],[40,-18],[18,-35],[0,-5],[-17,5],[-18,15]]);
  const asia = pts([[40,43],[60,55],[90,55],[120,53],[145,58],[155,45],[140,35],[125,22],[105,10],[90,20],[75,10],[55,20],[40,28],[40,43]]);
  const oceania = pts([[113,-12],[135,-12],[153,-25],[145,-38],[118,-35],[113,-22],[113,-12]]);

  return '<polygon ' + style + ' points="' + nAmerica + '"/>' +
    '<polygon ' + style + ' points="' + sAmerica + '"/>' +
    '<polygon ' + style + ' points="' + europe + '"/>' +
    '<polygon ' + style + ' points="' + africa + '"/>' +
    '<polygon ' + style + ' points="' + asia + '"/>' +
    '<polygon ' + style + ' points="' + oceania + '"/>';
}
